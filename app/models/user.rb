class User < ApplicationRecord
  ...
  def tweet!(message, url=ENV["DEFAULT_SHARE_URL"])
    twitter_client.update truncated_message_with_url(message, url)
  end

  def fb_post!(message, name= ENV["DEFAULT_FB_POST_NAME"],
    description="TODO", url=ENV["DEFAULT_SHARE_URL"], img=ENV["DEFAULT_FB_SHARE_IMAGE"])
    options =
          {
            'access_token' => user_tokens.facebook.token,
            'message' => message,
            'link' => url,
            'picture' => img,
            'name' => name,
